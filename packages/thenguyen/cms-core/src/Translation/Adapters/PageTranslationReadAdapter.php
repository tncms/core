...
/**
 * Read-only compatibility adapter that resolves a Page's localized fields through
 * the Translation Platform (the Phase 9.0B `content_relational` driver) while
 * reproducing the legacy read semantics **exactly** (Phase 9.1C).
 *
 * It is the Pages analog of {@see PostTranslationReadAdapter}: Pages are the same
 * Content model (type=page), the same `cms_content_translations` store, and the
 * same `content_relational` driver as Posts — they differ only by `content.type`
 * and by their own feature flag. This adapter therefore mirrors, field for field,
 * how the CMS reads a page today —
 *
 ...
 * legacy uses. It is engaged only when `translation.modules.pages.read_driver` is
 * `read` (or `adapter`); with the default `legacy` it is dormant.
 */
final class PageTranslationReadAdapter
{
    ...
    {
        // Pages have a single read flag (`read_driver`); unlike Posts there is no
        // legacy `driver` back-compat alias to honour. 'read' and 'adapter' are
        // equivalent-active; anything else falls back to 'legacy'.
        $mode = (string) config('translation.modules.'.self::MODULE.'.read_driver', 'legacy');

        return in_array($mode, ['legacy', 'read', 'adapter'], true) ? $mode : 'legacy';
    }

    /** Whether Pages should be read through this adapter (flag is not 'legacy'). */
    ...
    public function title(Content $page, ?string $locale = null): string
    {
        $record = $this->firstAvailableRecord($page, $this->locale($locale));
    ...
    public function slug(Content $page, ?string $locale = null): string
    {
        $record = $this->firstAvailableRecord($page, $this->locale($locale));
    ...
    public function localeSlug(Content $page, string $locale): ?string
    {
        $record = $this->strictRecord($page, $locale);
        $slug = $record?->field('slug');

        return ($slug !== null && $slug !== '') ? (string) $slug : null;
    }

    /** Whether the page has a row for the exact locale (mirrors Content::hasTranslation). */
    public function hasTranslation(Content $page, string $locale): bool
    {
        if ($page->relationLoaded('translations')) {
            return $page->translations->contains('locale', $locale);
        }

        return $this->driver->exists($page->getKey(), $locale);
    }

    /** Strict-locale excerpt (nullable) — mirrors ContentManager::getTranslation. */
    public function excerpt(Content $page, ?string $locale = null): ?string
    {
        return $this->strictField($page, $this->locale($locale), 'excerpt');
    }

    /** Strict-locale content body (nullable). */
    public function content(Content $page, ?string $locale = null): ?string
    {
        return $this->strictField($page, $this->locale($locale), 'content');
    }

    /** Strict-locale SEO title (meta_title, nullable). */
    public function seoTitle(Content $page, ?string $locale = null): ?string
    {
        return $this->strictField($page, $this->locale($locale), 'meta_title');
    }

    /** Strict-locale SEO description (meta_description, nullable). */
    public function seoDescription(Content $page, ?string $locale = null): ?string
    {
        return $this->strictField($page, $this->locale($locale), 'meta_description');
    }

    /** All six deliverable fields resolved for one page + locale. */
    public function fields(Content $page, ?string $locale = null): PageLocalizedFields
    {
        return new PageLocalizedFields(
            $this->title($page, $locale),
            $this->slug($page, $locale),
            $this->excerpt($page, $locale),
            $this->content($page, $locale),
            $this->seoTitle($page, $locale),
            $this->seoDescription($page, $locale),
        );
    }
    ...
    /**
     * Resolve fields for many page ids in ONE driver query — for callers holding
     * ids without loaded models. (The model-wired path already avoids N+1 by
     * reading the eager-loaded `translations` relation.)
     *
     * @param  array<int, int>  $pageIds
     * @return array<int, PageLocalizedFields>
     */
    public function batchFields(array $pageIds, ?string $locale = null): array
    {
        $loc = $this->locale($locale);
        $byId = $this->driver->batchRead($pageIds); // one query
     ...
     */
    private function firstAvailableRecord(Content $page, string $locale): ?TranslationRecord
    {
        if ($page->relationLoaded('translations')) {
        ...
        $record = $this->driver->read($page->getKey(), $locale);
        if ($record !== null) {
            return $record;
        }

        $all = $this->driver->readAllLocales($page->getKey());
     ...
     */
    private function strictRecord(Content $page, string $locale): ?TranslationRecord
    {
        if ($page->relationLoaded('translations')) {
            $translation = $page->translations->firstWhere('locale', $locale);
        ...
        return $this->driver->read($page->getKey(), $locale);
    }

    private function strictField(Content $page, string $locale, string $field): ?string
    {
        $value = $this->strictRecord($page, $locale)?->field($field);
     ...
     */
    private function composeFromMap(int $pageId, string $locale, array $map): PageLocalizedFields
    {
        $firstAvailable = $map[$locale] ?? (reset($map) ?: null);
        $strict = $map[$locale] ?? null;

        $title = $firstAvailable?->field('title');
        $title = ($title !== null && $title !== '') ? (string) $title : 'Content #'.$pageId;
