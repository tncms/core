...
/**
 * Read-only-of-business-rules WRITE compatibility adapter for Posts (Phase 9.0D).
 *
 * It performs exactly ONE thing: persist a post's translation row (create or
 ...
 *
 * The adapter therefore owns NO business behaviour:
 ...
 *
 * Engaged only when `translation.modules.posts.write_driver = adapter`; with the
 * default `legacy` it is dormant and ContentManager keeps its Eloquent write.
 */
final class PostTranslationWriteAdapter
{
    ...
    /** Whether Post translation writes should route through this adapter. */
    ...
    /**
     * Persist one already-normalized post translation through the driver, joining
    ...
    public function persist(Content $content, string $locale, array $payload, ?ContentTranslation $existing): ContentTranslation
    {
        ...
        if ($model === null) {
            throw new TranslationWriteException(
                'Post translation was not persisted for locale "'.$locale.'".'
            );
        }
    ...
    /**
     * Delete a post's translation rows through the driver (one locale, or all).
     *
     * NOT wired into ContentManager::delete in Phase 9.0D — see the delete-flow
     * decision in the report (Post deletion stays on the legacy soft-delete path).
     * Provided and tested for completeness and future use. Joins the caller's
     * transaction; returns rows removed.
     */
