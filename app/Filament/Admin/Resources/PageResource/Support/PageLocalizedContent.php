...
namespace App\Filament\Admin\Resources\PageResource\Support;
...
/**
 * Phase 9.1E — Pages admin adoption of the Translation Platform (Phase 8.3/8.4).
 *
 * The Pages analog of {@see \App\Filament\Admin\Resources\PostResource\Support\PostLocalizedContent}:
 * the Filament-free seam between the Pages admin editor and the Translation
 * Platform. It NEVER writes — it only sources locale ordering, validation
 * rules/messages, slug previews, and fallback previews. All writes stay with
 * ContentManager (see EditPage/CreatePage -> ContentManager ->
 * PageTranslationWriteAdapter -> ContentTranslationDriver).
 *
 * The Pages editor keeps its single-locale-per-page model, so these helpers are
 * scalar (one locale at a time) rather than the all-locale nested state the
 * Phase 8.3 LocaleTabs wrappers expect.
 */
final class PageLocalizedContent
{
    /**
     * The localized content fields the Pages editor adopts, keyed by the
     * admin form field name (which matches the cms_content_translations column).
     *
     * Mirrors the six fields named in the Phase 9.1E spec (the same set as Posts,
     * and the same cms_content_translations columns). meta_keywords and the
     * non-translatable columns are intentionally excluded. `excerpt` is defined
     * here for platform parity even though the current Pages form does not render
     * an excerpt input (the editing experience is preserved unchanged).
     *
    ...
    {
        return [
            'title' => LocalizedFieldDefinition::title('title', ['max_length' => 255]),
            // Slug stays optional (auto-generated from the title when blank) and
            // is only format-validated; ContentManager owns slug/cms_slugs.
            'slug' => LocalizedFieldDefinition::slug('slug', ['required_default' => false, 'max_length' => 255]),
            'excerpt' => LocalizedFieldDefinition::excerpt('excerpt'),
            'content' => LocalizedFieldDefinition::richContent('content'),
            'meta_title' => LocalizedFieldDefinition::seoTitle('meta_title', ['max_length' => 255]),
            'meta_description' => LocalizedFieldDefinition::seoDescription('meta_description'),
        ];
    }
     ...
     * (empty by default) and is NOT synced with the languages an admin enables
     * in the UI. Sourcing from the registry would drop active languages from the
     * editor and compute the wrong "default locale" for validation. We still feed
     * this LocaleOptions to the platform components, so the components are adopted
     * without depending on the unsynced registry. No hardcoded locales.
     */
    ...
    /**
     * Whether the field is required for the given locale (default-locale
     * required, secondary optional — per the field's validation policy).
     */
    ...
    /**
     * A locale-aware slug generated from the given title, for preview only.
     * Delegates to the platform SlugManager; performs NO cms_slugs write or
     * reservation. Returns null when there is no usable title.
     */
    public static function generateSlug(?string $title, string $locale): ?string
    {
        ...
        $slug = self::manager()->slugs->generate($title, $locale);
     ...
     * the record's stored translations and the live (unsaved) value for the
     * current locale. Never persisted; used only to show editors what the
     * frontend would inherit when the current locale is empty.
     *
     * @param  string|null  $currentValue  the live form value for $locale (form takes precedence over the stored row)
     */
    public static function fallbackPreview(Content $record, string $field, string $locale, ?string $currentValue = null): FallbackPreviewResult
    {
        $values = [];
