...
namespace App\Filament\Admin\Resources\PostResource\Support;
...
/**
 * Phase 9.0E — Posts admin adoption of the Translation Platform (Phase 8.3/8.4).
 *
 * This class is the Filament-free seam between the Posts admin editor and the
 * Translation Platform. It NEVER writes: it only sources locale ordering,
 * validation rules/messages, slug previews, and fallback previews. All writes
 * stay with ContentManager (see EditPost/CreatePost -> ContentManager ->
 * PostTranslationWriteAdapter -> ContentTranslationDriver).
 *
 * The Posts editor keeps its single-locale-per-page model, so these helpers are
 * scalar (one locale at a time) rather than the all-locale nested state the
 * Phase 8.3 LocaleTabs wrappers expect.
 */
final class PostLocalizedContent
{
    /**
     * The localized content fields the Posts editor adopts, keyed by the
     * admin form field name (which matches the cms_content_translations column).
     *
     * Only the six fields named in the Phase 9.0E spec are platform-driven;
     * meta_keywords and non-translatable columns are intentionally excluded.
     *
